...
class Simulator:
    def __init__(self, screen, screen_width, screen_height,
                 ...
                 activations=False,
                 record_data=False,
                 replay_data_path="replay_data/replay.txt",
                 state_buf_path="state_buf/state_buf.txt",
                 sensors=False,
        ...
        for index in range(0, len(arc_points)):
            if offroad_edge_points[index] == arc_points[index]:
                ...
                if display_obstacle_on_sensor is True:
                    # FERGAL: want to denote this as black (0, 0, 0) note red (255, 0, 0)
                    pygame.draw.line(self.screen, (0, 0, 0), offroad_edge_points[index], arc_points[index], True)
                    pygame.draw.line(act_mask, (0, 0, 0), offroad_edge_points[index], arc_points[index], True)
                    # Grid filled
    ...
    def optimized_rear_sensor(self, act_mask, display_obstacle_on_sensor=False):
        ...
        radius = 300
        arc_points = get_arc_points(mid_of_rear_axle, radius, radians(-90 + self.car.angle), radians(90 + self.car.angle),
                                    self.sensor_size)
    ...
    def record_data_function(self, index, all_highway_traffic):
        """
        recording tab
